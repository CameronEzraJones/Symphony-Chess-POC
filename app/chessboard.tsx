import { canMoveQueen } from "./queen";
...
export function BoardView({ board = startingBoard(), selected = null, onSquare }: {
  ...
  return (
    <div
      ...
      role="group"
      aria-label="Chessboard: select a bishop or queen, then a highlighted destination."
    >
      {board.map((piece, index) => {
        const row = Math.floor(index / 8);
        const column = index % 8;
        const color = (row + column) % 2 === 0 ? "light" : "dark";
        const legal = selected !== null && (canMoveBishop(board, selected, index) || canMoveQueen(board, selected, index));
        return (
          <button
            type="button"
            key={index}
